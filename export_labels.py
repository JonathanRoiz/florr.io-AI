...
print(f"Connected! Scanning {len(all_tasks)} reviewed tasks...")
...
for task in all_tasks:
    ...
    # FIX: Safely extract the dictionary from the list wrapper
    ...
    if results:
        for item in results:
            if item.get('type') == 'rectanglelabels':
                ...
                if rectangle_labels:
                    # Handle if label is nested inside a list ['Bee']
    ...
    # Verify physical file existence before copying
    source_img_path = os.path.join(LOCAL_IMAGE_DIR, img_filename)
    if os.path.exists(source_img_path):
        # Save Text Label file (generates a clean 0-byte file if yolo_lines is empty)
...
print(f"Saved exactly {exported_count} matching image/label pairs (including empty backgrounds).")
